...
class RangeLimit:
    ...
    character_exclude_list: list[c.CHARACTERS] = field(default_factory=list)
    nullify_character_exclude_list: bool = False
...
def add_to_collection(meta_space_subset: MetaStateSubset) -> None:
...
def get_limit(
    ...
    for limit in meta_subspace.limits:
        if adjustment[0] in limit.motions_names and adjustment[1] in limit.header_subset:
            if character not in limit.character_exclude_list:
                return limit

            if limit.nullify_character_exclude_list:
                return replace(
                    limit,
                    min=0,
                    max=0,
...
add_to_collection(BASIC_STAND_A_B)
    ...
    limits=[
        ...
        *[
            RangeLimit(
                ...
                min=0,
                max=int(MAX_FRAME_NUMBERS.loc[motion_name]),
            )
            for motion_name in ATTACK_ACTIONS_ALL
        ],
